...
if __name__ == '__main__':
    ...
    img = Image.open('visuals/cat.jpg')
    img = to_tensor(img)
    img = img.mul_(2).sub_(1)
    ...
    all_img = all_img.clamp_(0, 1)
    grid = make_grid(all_img, nrow=5)
    grid_img = to_pil_image(grid).convert('RGB')
    # save the image
    grid_img.save('visuals/aug.png')
    print('Saved the augmented image to visuals/aug.png')