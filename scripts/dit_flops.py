    ...
    for name, config in configs.items():
        ...
        for cond_name, cond_config in conditioning.items():
            for patch_size in patch_sizes:
                for use_gemb in use_gembs:
                    config["gembed"] = use_gemb
                    flops = dit_flops(patch_size=patch_size, **config, **cond_config, **universal_kwargs)
                    print(f"DiT-{name} ({patch_size}x{patch_size}), {cond_name}: {flops / 1e9:.4f}G [Gemb: {use_gemb}]")
    print(f"================================")