...
class ViTMAEConfig(PretrainedConfig):
    ...
    Args:
        ...
        full_pix_loss (`bool`, *optional*, defaults to `False`): Whether or not to use the full pixel loss. If `False`, use the masked pixel loss. 
    Example:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.full_pix_loss = full_pix_loss
@dataclass
...
class ViTMAEPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            nn.init.xavier_uniform_(module.weight) # following official implementation
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def forward_loss(self, pixel_values, pred, mask, interpolate_pos_encoding: bool = False):
        ...
        """
        full_pixel_loss = self.config.full_pix_loss
...
class ViTMAEForPreTrainingwBottleNeck(ViTMAEPreTrainedModel):
    def __init__(self, config, mlp_layers:int = 0, mlp_ratio:float = 4, bottleneck_ratio:float = 4):
        super().__init__(config)
        self.config = config

        self.vit = ViTMAEModel(config)
        self.decoder = ViTMAEDecoder(config, num_patches=self.vit.embeddings.num_patches)
        self.compressor = SimpleMLP(config.hidden_size, mlp_layers, mlp_ratio, bottleneck_ratio)
        # Initialize weights and apply final processing
        self.post_init()
