...
class id_connector(base_connector):
    def __init__(self):
        super().__init__() # do not track running mean and var
...
class Downsample_with_Conv_Connector(base_connector):
    def __init__(self, split:int = 1, hidden_size:int = 512, layers:int = 1,  bottleneck_ratio: float = 16.0, remove_cls: bool = True, patch_as_input: bool = False, kernel_size:int = 3, final_norm: bool = False):
        bn = nn.BatchNorm2d(hidden_size//bottleneck_ratio, affine=False)
        super().__init__(bn) # track running mean and var
        self.split = split
        self.layers = layers
        self.hidden_size = hidden_size
...
class RandomUpsample_Connector(base_connector):
    def __init__(self, input_hidden_channel:int, output_hidden_channel:int, dummy_sample: bool = False):
        ...
        # input : [batch_size, input_hidden_channel, sqrt(patch_size), sqrt(patch_size)]
