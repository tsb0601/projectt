...
with torch.no_grad():
    ...
    print('forward xs recon (saved in visuals/sanity_forward_recon.png ):', forward_output.xs_recon.shape, forward_output.xs_recon.min(), forward_output.xs_recon.max())
    recon_image = ToPILImage()(forward_output.xs_recon[0].clamp(0., 1.))
    recon_image.save('./visuals/sanity_forward_recon.png')
    print("=" * 10, 'testing stage1 model forward', "=" * 10)
    ...
    print('recon:( saved in visuals/sanity_recon.png )', recon.shape, recon.min(), recon.max())
